import React from 'react'
import EasyCard from '@/components/EasyCard/EasyCard'
import { RadialBar, RadialBarConfig } from '@ant-design/charts'

type Props = {}

const SocialSourceModule: React.FC<Props> = (props) => {
	const data = [
		{
			name: 'X6',
			star: 297,
		},
		{
			name: 'G',
			star: 506,
		},
		{
			name: 'AVA',
			star: 805,
		},
		{
			name: 'G2Plot',
			star: 1478,
		},
		{
			name: 'L7',
			star: 2029,
		},
		{
			name: 'G6',
			star: 7100,
		},
		{
			name: 'F2',
			star: 7346,
		},
		{
			name: 'G2',
			star: 10178,
		},
	];
	const config: RadialBarConfig = {
		data,
		xField: 'name',
		yField: 'star',
		height: 337,
		maxAngle: 350,
		//最大旋转角度,
		radius: 0.8,
		innerRadius: 0.2,
		tooltip: {
			customContent: () => {
				return (
					<div>
						<p>{data[0].name}</p>
						<p>{data[0].star}</p>
					</div>
				)
			}
			// formatter: (datum: { star: any }) => ({
			// 	name: 'star数',
			// 	value: datum.star,
			// }),
		},
		colorField: 'star',
		color: ({ star }) => {
			if (star > 10000) {
				return '#6349ec';
			} else if (star > 1000) {
				return '#ff9300';
			}

			return '#ff93a7';
		},
		barBackground: {},
		barStyle: {
			lineCap: 'round',
		},
		annotations: [
			{
				type: 'html',
				position: ['50%', '50%'],
				html: (container, view) => {
					const coord = view.getCoordinate();
					const w = (coord as any).polarRadius * coord.innerRadius * 1.15;
					return `<div style="transform:translate(-50%,-46%)">
          <img width="${(w / 203) * 231
						}" height="${w}" alt="" src="https://gw.alipayobjects.com/zos/antfincdn/zrh%24J08soH/AntV%252520LOGO%2525202.png">
        </div>`;
				},
			},
		],
	};
	return (
		<div>
			<EasyCard title={(<div> SOCIAL SOURCE </div>)}>
				<RadialBar {...config} />
			</EasyCard>
		</div>
	)
}

export default SocialSourceModule